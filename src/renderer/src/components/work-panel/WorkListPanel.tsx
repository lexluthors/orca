...
import { Check, Trash2, ChevronDown, ListChecks, CalendarClock, StickyNote } from 'lucide-react'
...
import { useWorkStore, useMergedWorkItems, useTodoCounts } from './use-work-store'
import {
  addWorkHours,
  subtractWorkHours,
  addWorkDays,
...
import type { WorkItemPriority, TodoFilter, WorkItemSort } from './types'
import { MEMO_COLORS } from './types'
import { TodoRow, MemoRow } from './WorkItemRows'
...
export function WorkListPanel({
  ...
  const adjustDueDate = useCallback((type: 'hour' | 'day', delta: number) => {
    setDueDate((prev) => {
      if (type === 'hour') {
    ...
    <div className="flex h-full flex-col">
            ...
            <DropdownMenuContent align="end" className="w-28">
              {SORT_OPTIONS.map((opt) => (
                <DropdownMenuItem
                  ...
                  className={cn('text-xs', sort === opt.value && 'bg-primary/10 text-primary')}
                >
                  {opt.label}
                </DropdownMenuItem>
              ))}
      ...
      <div className="border-t border-border/50 px-4 py-2">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-foreground min-w-[120px]">{dueDateText}</span>
